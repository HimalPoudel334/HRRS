...
ALTER TABLE [BedCounts] DROP CONSTRAINT [FK_BedCounts_HospitalType_FacilityTypeSN];

DROP INDEX [IX_BedCounts_FacilityTypeSN] ON [BedCounts];

DECLARE @var2 sysname;
SELECT @var2 = [d].[name]
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[BedCounts]') AND [c].[name] = N'FacilityTypeSN');
IF @var2 IS NOT NULL EXEC(N'ALTER TABLE [BedCounts] DROP CONSTRAINT [' + @var2 + '];');
ALTER TABLE [BedCounts] DROP COLUMN [FacilityTypeSN];

CREATE TABLE [BedCountFacilityType] (
    [BedCountsId] int NOT NULL,
    [FacilityTypesSN] int NOT NULL,
    CONSTRAINT [PK_BedCountFacilityType] PRIMARY KEY ([BedCountsId], [FacilityTypesSN]),
    CONSTRAINT [FK_BedCountFacilityType_BedCounts_BedCountsId] FOREIGN KEY ([BedCountsId]) REFERENCES [BedCounts] ([Id]) ON DELETE CASCADE,
    CONSTRAINT [FK_BedCountFacilityType_HospitalType_FacilityTypesSN] FOREIGN KEY ([FacilityTypesSN]) REFERENCES [HospitalType] ([SN]) ON DELETE CASCADE
);

CREATE INDEX [IX_BedCountFacilityType_FacilityTypesSN] ON [BedCountFacilityType] ([FacilityTypesSN]);

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20250310024131_BedCountFacilityTypeMappings', N'9.0.1');

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20250310031306_AddBedC', N'9.0.1');

ALTER TABLE [RegistrationRequests] ADD [RoleId] int NOT NULL DEFAULT 0;

ALTER TABLE [HealthFacilities] ADD [RoleId] int NOT NULL DEFAULT 0;

CREATE INDEX [IX_RegistrationRequests_RoleId] ON [RegistrationRequests] ([RoleId]);

CREATE INDEX [IX_HealthFacilities_RoleId] ON [HealthFacilities] ([RoleId]);

ALTER TABLE [HealthFacilities] ADD CONSTRAINT [FK_HealthFacilities_UserRoles_RoleId] FOREIGN KEY ([RoleId]) REFERENCES [UserRoles] ([Id]) ON DELETE CASCADE;

ALTER TABLE [RegistrationRequests] ADD CONSTRAINT [FK_RegistrationRequests_UserRoles_RoleId] FOREIGN KEY ([RoleId]) REFERENCES [UserRoles] ([Id]) ON DELETE CASCADE;

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20250310140316_AddRoleColInFacility', N'9.0.1');
