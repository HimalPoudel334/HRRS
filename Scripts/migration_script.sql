...
CREATE TABLE [HospitalStandards] (
    ...
    [Has25] nvarchar(max) NULL,
    [Has50] nvarchar(max) NULL,
    [Has100] nvarchar(max) NULL,
    [Has200] nvarchar(max) NULL,
    [Remarks] nvarchar(max) NULL,
    [FilePath] nvarchar(max) NULL,
    [FiscalYear] nvarchar(max) NULL,
    [Status] bit NULL,
    CONSTRAINT [PK_HospitalStandards] PRIMARY KEY ([Id]),
    CONSTRAINT [FK_HospitalStandards_HealthFacilities_HealthFacilityId] FOREIGN KEY ([HealthFacilityId]) REFERENCES [HealthFacilities] ([Id]) ON DELETE CASCADE,
...
CREATE INDEX [IX_HospitalStandards_HealthFacilityId] ON [HospitalStandards] ([HealthFacilityId]);
...
VALUES (N'20250209090855_InitialCrreate', N'9.0.1');

DECLARE @var sysname;
SELECT @var = [d].[name]
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[HospitalStandards]') AND [c].[name] = N'Status');
IF @var IS NOT NULL EXEC(N'ALTER TABLE [HospitalStandards] DROP CONSTRAINT [' + @var + '];');
ALTER TABLE [HospitalStandards] DROP COLUMN [Status];

ALTER TABLE [SubMapdandas] ADD [Status] bit NOT NULL DEFAULT CAST(0 AS bit);

ALTER TABLE [Mapdandas] ADD [Status] bit NOT NULL DEFAULT CAST(0 AS bit);

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20250209152703_AddedStatusInMapdanda', N'9.0.1');

ALTER TABLE [Mapdandas] ADD [Group] nvarchar(max) NULL;

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20250211075327_AddMapdandaGroup', N'9.0.1');

DECLARE @var1 sysname;
SELECT @var1 = [d].[name]
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[HospitalStandards]') AND [c].[name] = N'Has50');
IF @var1 IS NOT NULL EXEC(N'ALTER TABLE [HospitalStandards] DROP CONSTRAINT [' + @var1 + '];');
ALTER TABLE [HospitalStandards] ALTER COLUMN [Has50] bit NULL;

DECLARE @var2 sysname;
SELECT @var2 = [d].[name]
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[HospitalStandards]') AND [c].[name] = N'Has25');
IF @var2 IS NOT NULL EXEC(N'ALTER TABLE [HospitalStandards] DROP CONSTRAINT [' + @var2 + '];');
ALTER TABLE [HospitalStandards] ALTER COLUMN [Has25] bit NULL;

DECLARE @var3 sysname;
SELECT @var3 = [d].[name]
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[HospitalStandards]') AND [c].[name] = N'Has200');
IF @var3 IS NOT NULL EXEC(N'ALTER TABLE [HospitalStandards] DROP CONSTRAINT [' + @var3 + '];');
ALTER TABLE [HospitalStandards] ALTER COLUMN [Has200] bit NULL;

DECLARE @var4 sysname;
SELECT @var4 = [d].[name]
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[HospitalStandards]') AND [c].[name] = N'Has100');
IF @var4 IS NOT NULL EXEC(N'ALTER TABLE [HospitalStandards] DROP CONSTRAINT [' + @var4 + '];');
ALTER TABLE [HospitalStandards] ALTER COLUMN [Has100] bit NULL;

ALTER TABLE [HospitalStandards] ADD [CreatedAt] datetime2 NOT NULL DEFAULT '0001-01-01T00:00:00.0000000';

ALTER TABLE [HospitalStandards] ADD [Status] bit NOT NULL DEFAULT CAST(0 AS bit);

ALTER TABLE [HospitalStandards] ADD [UpdatedAt] datetime2 NULL;

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20250213024247_AddStatusAndDateTimeColumn', N'9.0.1');

ALTER TABLE [HospitalStandards] ADD [StandardEntryId] int NOT NULL DEFAULT 0;
...
ALTER TABLE [HospitalStandards] ADD CONSTRAINT [FK_HospitalStandards_HospitalStandardEntrys_StandardEntryId] FOREIGN KEY ([StandardEntryId]) REFERENCES [HospitalStandardEntrys] ([Id]) ON DELETE CASCADE;

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20250214035052_AddeEntryTable', N'9.0.1');
